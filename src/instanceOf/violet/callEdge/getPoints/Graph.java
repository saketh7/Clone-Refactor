...
public abstract class Graph implements Serializable
{
	...
	/**
      Graphs that needs sort nodes, should provide the comparator.
	...
	/*
      Since we are allowing trees, sort the nodes smartly, in the same order
      that added, but the fathers just before its childrens.
	 */
	private void sortNodes()
	{
		ArrayList sort = new ArrayList(nodes.size() + 1);
		for (int i = 0; i < nodes.size(); i++)
		{
			...
			{
				sort.add(node);
				sortChildren(sort, node);
			}
		}
		nodes = sort;
	}

	private void sortChildren(ArrayList sort, Node node)
	{
	...
	public static final Comparator sortY = new Comparator()
	{
		public int compare(Object o1, Object o2)
		{
			Node c1 = (Node) o1;
			Node c2 = (Node) o2;
			double diff = c1.getBounds().getY() - c2.getBounds().getY();
			if (diff < 0) return -1;
			if (diff > 0) return 1;
			return 0;
		}
